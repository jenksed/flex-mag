...
<?php global $do_not_duplicate; if (isset($do_not_duplicate)) { ?>
    <div class="feat-widget-cont left relative">
        ...
        $recent = tribe_get_events($args, true);
        while ($recent->have_posts()) :
            $recent->the_post();
            $count++; if ($count == 1 && $pop_ad) { ?>
            ...
            <div class="widget-ad left relative pop-ad">
                        <?php $pop_ad = get_option('mvp_pop_ad');
                        if ($pop_ad) {
                                echo html_entity_decode($pop_ad);
                        } ?>
            </div><!--widget-ad-->
            <?php } else { ?>
            <div class="feat-widget-wrap left relative">
...
<?php } else { ?>
    <div class="feat-widget-cont left relative">
        ...
        $pop_num = esc_html(get_option('mvp_pop_num'));
        $popular_days_ago = "$pop_days days ago";
        $recent = new WP_Query(array( 'post__not_in' => array( $post->ID ), 'posts_per_page' => $pop_num, 'orderby' => 'meta_value_num', 'order' => 'DESC', 'meta_key' => 'post_views_count', 'date_query' => array( array( 'after' => $popular_days_ago )) ));
        while ($recent->have_posts()) :
            $recent->the_post();
            $count++; if ($count == 1 && $pop_ad) { ?>
            <div class="feat-widget-wrap left relative">
